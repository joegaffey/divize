...
// Colour at a sample point: nearest seed, optionally blended toward the
// runner-up seed across cell boundaries (Macura/Faloutsos soft Voronoi).
...
fn fs_main(@builtin(position) p : vec4f) -> @location(0) vec4f {
  ...
  // seed space so the progress lines / scaled draws line up with the source.
  let base = vec2f(p.x, p.y) * params.scale;

  var c : vec4f;
  if (params.aa > 0u) {
    // 2x2 box anti-alias: average 4 sub-samples spaced 1/2 px apart.
