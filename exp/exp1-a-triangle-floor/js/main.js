/* ============================================================
 * divize – main.js  (exp1-a · floor-primitive showdown)
 * Loads an image, extracts saliency, samples adaptive seeds, runs
 * optional CVT, and renders one of three Layer-0 primitives:
 *   voronoi   pure nearest-seed floor (exp1 baseline)
 *   tri-tiled Delaunay triangle mesh (0x02) — flat or barycentric colours
 *   tri-splat center+transform α-triangles (0x03) over a backdrop
 * Includes SSIM/PSNR vs the source and a per-style byte-cost readout.
 * ============================================================ */
(function () {
  'use strict';
  ...
  const els = {
    ...
    splatAlphaOut: $('splatAlphaOut'), backdropSel: $('backdropSel'),
  ...
  function paintMap(canvas, field, w, h, heat) {
    ...
    for (let i = 0; i < w * h; i++) {
      ...
      if (heat) {
        ...
        d[o] = d[o + 1] = d[o + 2] = Math.round(field[i] * 255);
      }
      d[o + 3] = 255;
    }
  ...
  function primitiveCount() {
    const s = activeStyle();
    if (s === 'tri-tiled') return state.mesh ? state.mesh.tris.length : 0;
    if (s === 'tri-splat') return state.splats ? state.splats.length : 0;
  ...
  function backdropRGB() {
    const m = state.mean;
    return els.backdropSel.value === 'voronoi' ? null : m;
  }
  ...
  async function gpuRender(w, h) {
    ...
    try {
      ...
      } else if (s === 'tri-tiled' && state.mesh) {
        const m = state.mesh;
        ok = await window.divGPU.renderTri(m.verts, m.vertColors, m.tris, m.triColors,
          w, h, count, scale, m.interp, backdropFloat(), aa);
      } else if (s === 'tri-splat' && state.splats) {
        const vor = els.backdropSel.value === 'voronoi';
        ok = await window.divGPU.renderSplat(state.splats, state.pts, state.colors,
          w, h, count, scale, backdropFloat(), vor, aa);
      }
    } catch (e) {
      console.warn('[exp1-a] GPU render failed, falling back to CPU:', e);
      ok = false;
    }
    ...
    if (s === 'tri-splat' && state.splats) {
      ...
      return (X, Y) => {
        ...
        if (vor) {
          ...
          rr = colors[best * 4]; gg = colors[best * 4 + 1]; bb = colors[best * 4 + 2];
        }
        for (let k = 0; k < count; k++) {
          const sp = splats[k];
  ...
  // (never touches the possibly-WebGPU-bound display canvas).
  function renderCpuSync(w, h, count) {
    const sx = state.w / w;
  ...
  function caption(backend, style, count, np) {
    const names = {
      voronoi: 'Voronoi cells', 'tri-tiled': 'triangle mesh', 'tri-splat': 'α-triangle splats',
    };
    const n = names[style] || 'primitives';
    return `${backend} ${n} · ${count}/${np} drawn`;
  }
  ...
  function payloadBytes() {
    ...
    if (s === 'tri-splat' && state.splats) return 13 + state.splats.length * 8;
  ...
  els.backdropSel.addEventListener('change', scheduleRenderWork);
  ...
  async function runPipelineInner(token, stillCurrent) {
    ...
    if (it > 0) {
      ...
      if (gpuCvt) {
        ...
        try {
          out = await window.divGPU.cvt.run(pointsDown(), fieldRun, cw, ch, it, {
          ...
          state.cvtDown = true;
          console.warn('[exp1-a] GPU CVT failed, disabling:', e);
        }
      }
      if (out === null && stillCurrent()) {
        showWork(true, 'CVT relax (CPU)…');
        out = await se.cvtRelaxAsync(pointsDown(), fieldRun, cw, ch, it, {
          onProgress: (done, total) =>
    ...
    if (s === 'tri-tiled') {
      ...
      state.mesh = se.triangleMesh(pts, state.colors, state.buf.data, w, h,
        state.field !== null ? state.field : ones(w, h), els.triColor.value === 'interp',
        { frame: 'midpoints' });
      showWork(false);
    } else if (s === 'tri-splat') {
      showWork(true, 'splat sizing…');
      state.splats = se.splatParams(pts, state.buf.data, w, h, +els.splatAlpha.value);
      showWork(false);
    }

    tCvt = performance.now() - t0 - tSample;
    els.statMs.textContent = `s ${tSample.toFixed(0)} / c ${tCvt.toFixed(1)}ms`;
